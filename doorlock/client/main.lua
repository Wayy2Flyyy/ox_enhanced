local Marker = require 'client.marker'

if not LoadResourceFile(cache.resource, 'web/build/index.html') then
	error('Unable to load UI. Build doorlock or download the latest release.\n	^3https://github.com/Wayy2Flyyy/ox_enhanced/releases/latest/download/doorlock.zip^0')
end
...
CreateThread(function()
	local showDoorPrompt = Config.DrawSprite
		...
		if num > 0 then
			for i = 1, num do
				...
				if door.distance < door.maxDistance then
					...
					if showDoorPrompt and not door.hideUi then
						Marker.drawEKey(door.coords, door.state)
					end
				end
			end
