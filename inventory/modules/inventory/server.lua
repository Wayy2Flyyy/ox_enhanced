...
---@alias updateSlot { item: SlotWithItem | { slot: number }, ox_inventory: string|number }
...
local function hasActiveInventory(playerId, owner)
	...
	if activePlayer then
		...
		if inventory then
			...
			db.savePlayer(owner, json.encode(ox_inventory:minimal()))
...
RegisterCommand('clearActiveIdentifier', function(source, args)
    ...
    Inventory.CloseAll(inventory)
    db.savePlayer(inventory.owner, json.encode(ox_inventory:minimal()))
    Inventory.Remove(inventory)
end, true)
...
local inventoryClearTime = GetConvarInt('ox_inventory:cleartime', 5) * 60
			...
			if item.metadata.components then
				for k, v in pairs(item.metadata.components) do
					if v == value.component then
						...
						ox_inventory:syncSlotsWithPlayer({
							{ item = item }
						}, inventory.weight)
